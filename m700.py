...
class M700():
    ...
    def __open(self):
        ...
        if not self.__isopen:
            ...
            errcd = self.__ezcom.Open2(5, self.__unitno, 30, 'EZNC_LOCALHOST')
            self.__raise_error(errcd)
            self.__isopen = True
    ...
    def set_tool_offset_h(self, toolset_no, h):
        ...
        with self.__lock:
            ...
            self.__raise_error(errcd)
            errcd = self.__ezcom.Tool_SetOffset(4, 2, toolset_no, h, 0)
            self.__raise_error(errcd)
    ...
    def get_tool_life(self,tool_group,tool_no):
        '''return the life value of toolset'''
        with self.__lock:
            self.__open()
            errcd, count = self.__ezcom.Tool_GetToolLifeValue(tool_group,tool_no)
            self.__raise_error(errcd)

            return count
    
    def get_tool_list(self,tool_group):
        '''return the life value of toolset'''
        with self.__lock:
            self.__open()
            errcd, toollist = self.__ezcom.Tool_GetToolLifeToolNoList(tool_group,tool_group)
            self.__raise_error(errcd)
            # self.__ezcom.CoTaskMemFree(toollist,newdata)
            return toollist
    ...
    #add by karel 20230510
    def read_commv(self,var:int):
        '''get the value of the common parameters 
        Args:
            var(int): common variants exp:#100,#500
            c70:#100~#199,#500~#999
            m700/m800:100~199,400~499,100100~100199,200100~200199,300100~300199,
            400100~400199,500100~500199,600100~600199,700100~700199,800100~800199,900000~907399
        '''
        with self.__lock:
            self.__open()
            pdData = 0
            pItype = 0
            errcd,value,type = self.__ezcom.CommonVariable_Read2(var,pdData,pItype)
            self.__raise_error(errcd)
            # print(res)
            return value
    ...
    def write_commv(self,var:int,data:float):
        '''
            set the value of the common variable exp:#100,#500 without #
        Args:
            var:the number of the common variable
            data: the new value for the variable.
        return:
            bool:true: success to write
        '''
        with self.__lock:
            self.__open()
            itype = 1 # number type
            errcd = self.__ezcom.CommonVariable_Write2(var,data,itype)
            self.__raise_error(errcd)
            return True
